(typeof define === 'function' && define.amd ? define : /* istanbul ignore next */ function (factory) {
	...
	var getSeleniumKey = (function () {
		...
		return function (key, keyLocation, isUpperCase) {
			if (keyLocation === /* numpad */ 3 && NUMPAD_KEY_MAP[key]) {
			...
			if (key.slice(0, 2) === 'U+') {
				var char = String.fromCharCode(Number('0x' + key.slice(2)));
	...
	Recorder.prototype = {
		...
		_eraseThrough: function (methodName) {
			var commands = this._currentTest.commands;
		...
		_getDefaultHotkeys: function () {
			return {
				'insertCallback': { altKey: true, shiftKey: true, key: /* c */ 'U+0043' },
				'insertMouseMove': { altKey: true, shiftKey: true, key: /* m */ 'U+004D' },
			...
			this.chrome.runtime.onConnect.addListener(function (port) {
				...
				port.onDisconnect.addListener(function disconnect() {
					...
					/* istanbul ignore else: there are only two port types */
						...
						self._contentPort = self._getDefaultPort();
					}
				});
		...
		_injectContentScript: function () {
			...
			this.chrome.tabs.executeScript(this.tabId, { file: 'lib/content.js', allFrames: true });
		},
		...
		_recordKey: function (event) {
			...
			var key = getSeleniumKey(event.key, event.location, event.shiftKey);
		...
		save: function () {
			...
			xhr.open("GET", "file:///C:/mitrend2/theintern/recorder/", true);
